...
class VoiceConverter:
    ...
    def convert(self, source_path, target_path, output_path, use_npy=False, emotion_label=None):
        ...
        with torch.no_grad():
            ...
            base_name = f"mel_{src_name}_to_{tgt_name}"

            # === Save PNG ===
            plot_path = os.path.join(os.path.dirname(output_path), f"{base_name}.png")
            plt.savefig(plot_path)
            plt.close()
            print(f"Saved AutoVC mel-spectrogram to {plot_path}")

            # === Save NPY ===
            mel_npy_path = os.path.join(os.path.dirname(output_path), f"{base_name}.npy")
            np.save(mel_npy_path, mel_out_np)
            print(f"Saved AutoVC mel as .npy to {mel_npy_path}")
